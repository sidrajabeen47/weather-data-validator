...
# def insert_weather_data(city_name, country, temp, humidity, wind_speed, condition, feels_like, pressure, visibility, uv_index, raw_text):
...
#         ) VALUES (%s, %s, %s, %s, %s, %s, CURDATE(), CURTIME(), %s, %s, %s, %s, %s)
#     """
#     data_tuple = (
#         city_name, country, temp, humidity, wind_speed, condition,
#         feels_like, pressure, visibility, uv_index, raw_text
#     )
...
#         insert_weather_data(city_name, country, temp, humidity, wind_speed, condition, feels_like, pressure, visibility, uv_index, raw_text)
...
#     sql_query = "SELECT id, city, temperature, `condition`, search_date, search_time FROM weather_reports"
...
#     else:
#         print(f"\n{'ID':<5} | {'City':<15} | {'Temp':<6} | {'Condition':<15} | {'Date':<12} | {'Time':<8}")
#         print("-" * 72)
#         for row in dataset:
#             print(f"{row[0]:<5} | {row[1]:<15} | {row[2]:.1f}°C | {row[3]:<15} | {str(row[4]):<12} | {str(row[5]):<8}")
...
#     sql_query = "SELECT id, city, temperature, `condition`, search_date, search_time FROM weather_reports ORDER BY id DESC LIMIT 1"
#     cursor.execute(sql_query)
...
#         print(f"ID: {record[0]} | City: {record[1]} | Temp: {record[2]}°C | Status: {record[3]} | Checked: {record[4]} at {record[5]}")
...
#     sql_query = f"SELECT city, temperature, `condition` FROM weather_reports ORDER BY temperature {sort_order} LIMIT 1"
    
#     cursor.execute(sql_query)
#     record = cursor.fetchone()
    
#     if not record:
#         print("\nHistory tracking database is empty.")
#     else:
...
#     sql_query = "SELECT COUNT(*), MAX(temperature), MIN(temperature), AVG(temperature) FROM weather_reports"
#     cursor.execute(sql_query)
#     stats = cursor.fetchone()

    
#     if stats[0] == 0:
#         print("\nNo tracking entries currently saved to analyze.")
#     else:
#         print("\n=========================================")
...
#     sql_query = "SELECT city, temperature, `condition` from weather_reports"
#     cursor.execute(sql_query)
#     dataset = cursor.fetchall()
...
#     else:
#         target_file = open("weather_history.txt", "w", encoding="utf-8")
#         for row in dataset:
#             target_file.write(f"{row[0]} | {row[1]}°C | {row[2]}\n\n")
#         target_file.close()
...
# def main():
#     """Handles continuous menu execution loop."""
#     while True:
#         print("\n" + "="*40)
...
#             break
#         else:
#             print("\nInvalid input choice. Please select an option from 1 to 9.")
...
#     # Track metrics if anything components drop issues
#     if not all_passed:
...
#             print("Weather report successfully saved to database history.")
#         else:
#             # Task 9: Handle failures safely without database injection crashes
#             print("\nValidation Failed")
#             print("Record Not Saved")
            
#             # Bonus Challenge 2 & 3 Process pipelines 
#             write_validation_log(city_name, "FAILED")
#             export_invalid_record(city_name, country, temp, humidity, wind_speed, condition)
            
#             # If your app requires keeping historical tabs on FAILED payload searches inside the db:
#             # insert_weather_data(city_name, country, temp, humidity, wind_speed, condition, feels_like, pressure, visibility, uv_index, raw_text, "FAILED")
#     else:
#         print("Could not retrieve weather details. Please verify city name spelling.")
...
#     # Included validation_status selection array string mapping 
...
#     sort_order = "DESC" if metric_type == "Hottest" else "ASC"
#     # Filter by PASSED records to verify metrics remain untainted by corrupted entries
...
#     # Calculate statistics only from valid insertions
...
#             delete_weather_history()
#         elif choice == "9":
#             # Feature 3 Output Metric Interface display
...
#     # CRITICAL FIX: Validate the USER'S processed string BEFORE pulling API resources
